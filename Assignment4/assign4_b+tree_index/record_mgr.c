...
typedef struct TableInfo {
    ...
    int maxRecordsPerPage;      // 每个页面最多能容纳的记录数
...
// ------------------------------------
// 辅助函数：初始化数据页
static RC initDataPage(BM_BufferPool *bp, BM_PageHandle *ph, int maxRecordsPerPage) {
    ...
    // 预分配槽位目录空间
    memset(ph->data + header->slotDirOffset, 0, maxRecordsPerPage * sizeof(SlotDirEntry));
...
// 辅助函数：在数据页中查找空闲槽位
static RC findFreeSlotInPage(BM_BufferPool *bp, BM_PageHandle *ph, int *slotNum, int maxRecordsPerPage) {
    PageHeader *header = (PageHeader *)ph->data;
    ...
    // 检查是否还有空间添加新槽位
    if (header->slotCount < maxRecordsPerPage) {
...
static RC insertRecordIntoSlot(BM_BufferPool *bp, BM_PageHandle *ph, int slotNum, Record *record, int recordSize) {
    ...
    // 固定的页面布局计算
    int slotDirSize = header->slotCount * sizeof(SlotDirEntry);
    int dataAreaStart = header->slotDirOffset + slotDirSize;
...
RC createTable (char *name, Schema *schema)
{
    ...
    int slotDirEntrySize = sizeof(SlotDirEntry);
    tableInfo.maxRecordsPerPage = (PAGE_SIZE - pageHeaderSize) / (slotDirEntrySize + tableInfo.recordSize);
    // 确保记录大小不超过页面大小
    if (tableInfo.recordSize > PAGE_SIZE - pageHeaderSize - slotDirEntrySize) {
        tableInfo.maxRecordsPerPage = 0;
        return RC_RM_RECORD_TOO_LARGE;
    }
...
RC insertRecord(RM_TableData *rel, Record *record) {
    ...
    if (header->slotCount == 0) {
        initDataPage(bp, &ph, mgmt->tableInfo.maxRecordsPerPage);
    }
    
    // 4. 查找空闲槽位
    int slotNum = -1;
    rc = findFreeSlotInPage(bp, &ph, &slotNum, mgmt->tableInfo.maxRecordsPerPage);
    if (rc != RC_OK) {
        // 没有空闲槽位，需要创建新页面
...
RC updateRecord(RM_TableData *rel, Record *record) {
    ...
    // 统一的页面有效性检查，与getRecord函数保持一致
    if (record->id.page < 1 || record->id.page >= mgmt->tableInfo.totalPages) {
        DEBUG_PRINT("updateRecord: Page %d out of range (totalPages=%d)\n", 
                    record->id.page, mgmt->tableInfo.totalPages);
        return RC_INVALID_PARAMS;
    }
    ...
    // 检查槽位有效性，增加调试信息
    if (record->id.slot < 0 || record->id.slot >= header->slotCount) {
        DEBUG_PRINT("updateRecord: Invalid slot %d on page %d - slotCount=%d\n", 
                    record->id.slot, record->id.page, header->slotCount);
        releasePageToBuffer(bp, &ph, FALSE);
        return RC_RM_NO_MORE_TUPLES;
    }
    
    if (!slotDir[record->id.slot].isValid) {
        DEBUG_PRINT("updateRecord: Slot %d on page %d is not valid\n", 
...
/**
 * @brief start a scan on a table
 * 
 * @param rel, input parameter, pointer to the table data
 * @param scan, output parameter, pointer to the scan handle
 * @param cond, input parameter, pointer to the condition expression
 * @return RC_OK
 */
RC startScan (RM_TableData *rel, RM_ScanHandle *scan, Expr *cond)
{
    if (rel == NULL || scan == NULL || rel->mgmtData == NULL) 
        return RC_INVALID_PARAMS;

    // 分配扫描管理数据
    ScanMgmtData *scanData = (ScanMgmtData *)malloc(sizeof(ScanMgmtData));
    if (scanData == NULL) return RC_OUT_OF_MEMORY;

    // 初始化扫描状态
    scanData->cond = cond;  // 条件表达式（外部管理）
    scanData->currentPage = 1;  // 从第1页开始（第0页是表信息）
    scanData->currentSlot = -1; // 从-1开始，next时会增加到0
    scanData->isEnd = FALSE;
    scanData->slotDir = NULL;
    scanData->slotDirSize = 0;
...
/**
 * @brief get the next record from a scan
 * 
 * @param scan, input parameter, pointer to the scan handle
 * @param record, output parameter, pointer to the record
 * @return RC_OK
 */
RC next (RM_ScanHandle *scan, Record *record)
{
    ...
    BM_BufferPool *bp = &mgmt->bufferPool;
    int recordSize = mgmt->tableInfo.recordSize;
    Schema *schema = mgmt->schema;

    // 如果扫描已结束，返回RC_RM_NO_MORE_TUPLES
    if (scanData->isEnd) {
        return RC_RM_NO_MORE_TUPLES;
    }
    ...
    // 遍历所有页面和槽位
    while (scanData->currentPage < mgmt->tableInfo.totalPages) {
        // 获取当前页面
        BM_PageHandle ph;
        RC rc = pinPage(bp, &ph, scanData->currentPage);
        if (rc != RC_OK) {
            // 页面无法访问，继续下一页
            scanData->currentPage++;
            scanData->currentSlot = -1;
            continue;
        }
        ...
        // 查找下一个有效记录
        for (scanData->currentSlot++; scanData->currentSlot < header->slotCount; scanData->currentSlot++) {
            if (slotDir[scanData->currentSlot].isValid) {
                // 创建临时记录用于条件检查
                Record tempRecord;
                tempRecord.id.page = scanData->currentPage;
                tempRecord.id.slot = scanData->currentSlot;
                tempRecord.data = ph.data + slotDir[scanData->currentSlot].offset;

                // 检查条件（如果有）
                if (scanData->cond == NULL) {
                    // 无条件扫描，直接返回记录
                    memcpy(record->data, tempRecord.data, recordSize);
                    ...
                    // 有条件扫描，计算表达式结果
                    Value *result = NULL;
                    rc = evalExpr(&tempRecord, schema, scanData->cond, &result);
                    if (rc == RC_OK && result != NULL && result->dt == DT_BOOL && result->v.boolV == TRUE) {
                        // 找到符合条件的记录，复制数据并返回
                        memcpy(record->data, tempRecord.data, recordSize);
                        record->id = tempRecord.id;
                        freeVal(result);  // 释放结果值
                        unpinPage(bp, &ph);
                        return RC_OK;
                    }
                    if (result != NULL) {
...
RC closeScan (RM_ScanHandle *scan)
{
    ...
    // 释放扫描管理数据
    ScanMgmtData *scanData = (ScanMgmtData *)scan->mgmtData;
    if (scanData->slotDir != NULL) {
        free(scanData->slotDir);
    }
    free(scanData);
