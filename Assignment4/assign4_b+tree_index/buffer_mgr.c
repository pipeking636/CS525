...
static RC  flushFrame(BM_BufferPool *bm, int frameIdx) {
    ...
    if (frame->isDirty) {
        ...
        if (frame->pageHandle.pageNum < 0 || frame->pageHandle.pageNum > MAX_PAGE_NUM) {
            // 假设合理的页码不会超过100万
        ...
        if (mgmt->fileHandle.totalNumPages <= 0 || mgmt->fileHandle.totalNumPages > MAX_PAGE_NUM) {
